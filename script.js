...
class CInterpreter {
  ...
  _lval(e,frame){
    ...
    if(e.type==='sub'){
      ...
      if(ref){ const cur=ref.get(); if(Array.isArray(cur))return{get:()=>cur[idx]??0,set:v=>{cur[idx]=v;}};
        if(typeof cur==='string'&&this._heap[cur]){ const blk=this._heap[cur]; if(!blk.arr)blk.arr=[]; return{get:()=>blk.arr[idx]??0,set:v=>{blk.arr[idx]=v;blk.data[idx]=v;}}; }
      }
      const base=this._eval(e.x,frame);
      if(Array.isArray(base))return{get:()=>base[idx]??0,set:v=>{base[idx]=v;}};
      if(typeof base==='string'&&this._heap[base]){ const blk=this._heap[base]; if(!blk.arr)blk.arr=[]; return{get:()=>blk.arr[idx]??0,set:v=>{blk.arr[idx]=v;blk.data[idx]=v;}}; }
    }
    if(e.type==='deref'){
      const ptr=this._eval(e.x,frame);
      if(this._addrCells&&this._addrCells[ptr]){ const cell=this._addrCells[ptr]; return{get:()=>cell.get(),set:v=>cell.set(v)}; }
      if(this._heap[ptr]){ if(!this._heap[ptr].arr)this._heap[ptr].arr=[]; return{get:()=>this._heap[ptr].arr[0]??0,set:v=>{this._heap[ptr].arr[0]=v;}}; }
