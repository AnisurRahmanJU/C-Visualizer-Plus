...
class CInterpreter {
  ...
  _parseStmt(){
    ...
    if(t.v==='struct'||this._isType(t.v)){
      ...
      try { return this._parseDecl(); }
        ...
        if(this._pk().v===','){
          throw new Error(`Expected ';' end of the line, but in the line - you can use same type of multiple variables with values using comma separation (line ${t.ln})`);
        }
        this._ex(';');
  ...
  _initGlobals(){
    for(const [name,g] of Object.entries(this.globals)){
      ...
      this._globalFrame.vars[name]={type:g.type,value:val,addr:this._nextAddr(),seg:g.init?'data':'bss',isConst:/\bconst\b/.test(g.type)};
    }
  }
  ...
  _callFn(name,args,callSite){
    ...
    fn.params.forEach((p,i)=>{
      ...
      frame.vars[p.name]={type:p.type,value:args[i]??0,addr:this._nextAddr(),isParam:true,seg:'stack',isConst:/\bconst\b/.test(p.type)};
  ...
  _execDecl(s,frame,opts={}){
    for(const d of s.decls){
      ...
      const entry={type:vt,value:val,addr:this._nextAddr(),changed:true,seg:s.isStatic?'static':'stack',_declNode:d,isConst:/\bconst\b/.test(vt)};
  ...
  _lval(e,frame){
    if(e.type==='id'){
      ...
      if(this._globalFrame.vars[n]!==undefined){
        ...
        return{get:()=>gf.vars[n].value,set:v=>{
          if(gf.vars[n].isConst) throw new Error(`The value of const variable '${n}' is unchangable, don't change it${e.ln?` (line ${e.ln})`:''}`);
          gf.vars[n].value=v;gf.vars[n].changed=true;
        }};
      }
    ...
    if(e.type==='deref'){
      ...
      for(const f of[...this._callStack,this._globalFrame]){if(!f)continue;for(const[k,v]of Object.entries(f.vars)){if(v.addr===ptr)return{get:()=>v.value,set:nv=>{
        if(v.isConst) throw new Error(`The value of const variable '${k}' is unchangable, don't change it`);
        v.value=nv;v.changed=true;
      }};}}
    }
    if(e.type==='mem'){
  ...
  _castVal(v,t){
    ...
    if(t.includes('bool')) return (Number(v)?1:0);
  ...
  // Lowered the step cap from 5000 down to 150. A correct, terminating
  // program (including the linked-list / linked-list-delete samples,
  // which only need a few dozen steps) will always finish well under this
  // limit, so 150 is more than enough for legitimate traces. The cap now
  // mainly exists as a safety net against runaway/infinite loops or
  // pathological recursion, and it kicks in much sooner than before so the
  // trace and the UI stay responsive instead of silently grinding out
  // thousands of steps.
  //
  ...
  _addStep(s){
    ...
    if(this.steps.length<150)this.steps.push(s);
  }
}
...
(function injectTypeColumnVisibilityFix(){
  ...
  const style = document.createElement('style');
  style.id = 'type-col-visibility-fix';
  style.textContent = `
    .vtbl th.col-type,
    .vtbl td.col-type {
    ...
    if (!entries.length) {
      ...
      for (const [name, v] of entries) {
        ...
        if (isArr) {
          ...
          else if (v.type && v.type.includes("bool") && typeof val === "number") {
            d = val ? "true" : "false";
        ...
        tr.innerHTML = `
          <td><span class="vn">${name}</span>${segBadge(v.seg)}${v.isConst ? '<span class="seg-badge" title="const — cannot be reassigned" style="background:var(--vsc-orange,#d19a66);color:#fff">const</span>' : ''}</td>
